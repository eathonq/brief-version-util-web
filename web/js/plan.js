async function init() {
    ...
    date.value = common.time_format(today, "yyyy-MM-dd");

    // 设置时间为当前时间
    const time = document.getElementById("projectTime");
    time.value = common.time_format(today, "hh:mm:ss");
...
async function planVersion() {
    ...
    // 切换版本
    const switchVersionUrl = `${common.version_server}/version/plan_switch/${projectName}/${projectVersion}/${date} ${time}`;
